 ...
 */
std::shared_ptr<CActualMachine> CMachineFactory2::CreateMachine()
{
    ...
    const int MotorX = -190;
    const int MotorY = -150;
    const int WinchX = -226;

    // Gear and post locations
    const int PostX = -250 + 59;
    const int PostY = -109;
    const int Gear2X = MotorX - 82;
    const int Gear2Y = MotorY - 56;
    const int Gear4X = PostX - 35;
    const int Gear4Y = PostY - 135;
    ...
    AddPost(machine, PostX, -PostY + 10);
    AddPost(machine, Gear4X - 48, -Gear4Y + 10);
    ...
    winch->SetPosition(Point(Gear2X - 33, Gear2Y - 26));
    winch->SetCableEnd(-46, -266);

    winch->SetTrap(trap);

    // The gear driven by the motor
    // Radius=25pixels, 12 teeth
    auto gear1 = make_shared<CGear>(25, 12);
    gear1->SetImage(L"images/hammered-copper.png");
    ...
    // The second gear
    // Radius=50pixels, 25 teeth
    auto gear2 = make_shared<CGear>(80, 45);
    ...
    auto gear4 = make_shared<CGear>(40, 25);
    gear4->SetColor(Color::Green);
    ...
    auto gear5 = make_shared<CGear>(45, 30);
    gear5->SetImage(L"images/hammered-copper.png");
    gear5->SetPosition(Point(Gear2X - 33,Gear2Y - 26));
    machine->AddComponent(gear5);
    gear5->SetAdjust(.004);
    // Direct connection
    gear4->Drive(gear5);

    gear5->GetSource()->AddSink(winch->GetSink());

    machine->AddComponent(winch);


    auto gear6 = make_shared<CGear>(45, 20);
    gear6->SetImage(L"images/hammered-copper.png");
    gear6->SetPosition(Point(Gear2X -2, Gear2Y + 119));
    ...
    // Flag that is driven by bottom gear
    auto flag = make_shared<CShape>();
    flag->AddPoint(0, 0);
    flag->AddPoint(0, -120);
    flag->AddPoint(60, -120);
    flag->AddPoint(60, 0);
    flag->SetImage(L"images/msu-flag.png");
    flag->SetPosition(gear6->GetPosition());
    machine->AddComponent(flag);
    gear6->GetSource()->AddSink(flag->GetSink());
 ...
 */
void CMachineFactory2::AddPost(std::shared_ptr<CActualMachine> machine, int x, int height)
{
    // The post itself
