...
BEGIN_MESSAGE_MAP(CViewEdit, CScrollView)
    ...
    ON_WM_LBUTTONDOWN()
    ON_WM_MOUSEMOVE()
    ON_COMMAND(ID_MACHINE_SETFIRSTMACHINE, &CViewEdit::OnMachineSetfirstmachine)
    ON_COMMAND(ID_MACHINE_SETSECONDMACHINE, &CViewEdit::OnMachineSetsecondmachine)
END_MESSAGE_MAP()
...
/**
* Opens a box to allow the first trap to be changed.
*/
void CViewEdit::OnMachineSetfirstmachine()
{
    GetPicture()->GetMachine1()->MachineImp();
    UpdateObserver();
}

/**
* Opens a box to allow the second trap to be changed.
*/
void CViewEdit::OnMachineSetsecondmachine()
{
    GetPicture()->GetMachine2()->MachineImp();
    UpdateObserver();
}
